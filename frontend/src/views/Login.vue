<template>
  <div class="max-w-5xl mx-auto px-4 py-10 md:py-14">
    <div class="panel overflow-hidden !rounded-3xl grid lg:grid-cols-2">
      <!-- SIDE / BRANDING -->
      <div class="relative overflow-hidden p-8 md:p-10 text-white flex flex-col justify-between min-h-[320px]"
        ...
        <div class="absolute inset-0 opacity-[0.12]" style="background-image: radial-gradient(#fff 1.2px, transparent 1.2px); background-size: 20px 20px;"></div>

        <div class="relative">
          <div class="flex items-center gap-3 mb-8">
            <span class="w-11 h-11 rounded-2xl bg-white/15 border border-white/30 backdrop-blur flex items-center justify-center">
              <i class="fa-solid fa-lock text-white text-lg"></i>
            </span>
            <div>
              <p class="font-display font-extrabold text-xl leading-none" dir="ltr">SecSim<span class="text-amber-300">_</span></p>
              <p class="text-teal-50/80 text-xs mt-1">{{ $t('hero.eyebrow') }}</p>
            </div>
          </div>

          <h2 class="font-display text-2xl md:text-3xl font-extrabold leading-snug mb-3">{{ $t('auth.side_title') }}</h2>
          <p class="text-teal-50/90 text-sm leading-relaxed mb-8 max-w-sm">{{ $t('auth.side_sub') }}</p>

          <ul class="space-y-3">
            <li v-for="(p, i) in $tm('auth.points')" :key="i"
              class="flex items-start gap-3 bg-white/10 border border-white/20 rounded-2xl px-4 py-3 backdrop-blur">
              <span class="w-9 h-9 rounded-xl bg-white/15 border border-white/25 flex items-center justify-center shrink-0">
        ...
        <div class="relative mt-8 flex flex-wrap items-center gap-2">
          <span class="inline-flex items-center gap-1.5 bg-white text-teal-800 text-xs font-extrabold px-3 py-1.5 rounded-full" dir="ltr">
      ...
      <div class="p-6 md:p-10 bg-white">
        <div class="grid grid-cols-2 gap-1 p-1 rounded-2xl bg-panel2 border border-[rgba(15,23,42,0.08)] mb-6">
          <button @click="mode = 'login'"
            :class="['py-2.5 rounded-xl text-sm font-bold transition', mode === 'login' ? 'bg-white shadow-card text-teal-700 border border-[rgba(15,23,42,0.08)]' : 'text-muted hover:text-mist']">
            <i class="fa-solid fa-right-to-bracket me-1.5"></i>{{ $t('auth.login') }}
          </button>
          <button @click="mode = 'register'"
            :class="['py-2.5 rounded-xl text-sm font-bold transition', mode === 'register' ? 'bg-white shadow-card text-teal-700 border border-[rgba(15,23,42,0.08)]' : 'text-muted hover:text-mist']">
        ...
        <div v-if="expiredNotice" class="flex items-start gap-2 bg-amber-50 border border-amber-300 text-amber-800 text-xs font-semibold rounded-xl px-3.5 py-3 mb-4">
        ...
        <form @submit.prevent="submit" class="space-y-4">
          <div v-if="mode === 'register'">
            ...
            <div class="relative">
              <i class="fa-solid fa-user absolute top-1/2 -translate-y-1/2 start-3.5 text-slate-400 text-sm pointer-events-none"></i>
              <input v-model="name" class="field w-full ps-10 pe-3 py-3 text-sm" autocomplete="name"
          ...
          <div>
            ...
            <div class="relative">
              <i class="fa-solid fa-envelope absolute top-1/2 -translate-y-1/2 start-3.5 text-slate-400 text-sm pointer-events-none"></i>
              <input v-model="email" type="email" required class="field w-full ps-10 pe-3 py-3 text-sm font-mono" dir="ltr"
                autocomplete="email" placeholder="you@mail.com" />
            </div>
          </div>
          ...
          <div>
            ...
            <div class="relative">
              <i class="fa-solid fa-key absolute top-1/2 -translate-y-1/2 start-3.5 text-slate-400 text-sm pointer-events-none"></i>
              <input v-model="password" :type="showPassword ? 'text' : 'password'" required minlength="8"
                class="field w-full ps-10 pe-16 py-3 text-sm font-mono" dir="ltr" autocomplete="current-password"
                placeholder="••••••••" />
          ...
          <div v-if="error" class="flex items-start gap-2 bg-red-50 border border-red-200 text-red-700 text-xs font-semibold rounded-xl px-3.5 py-3">
            <i class="fa-solid fa-circle-exclamation mt-0.5 shrink-0"></i>
            <span>{{ error }}</span>
          </div>

          <button type="submit" :disabled="loading" class="btn-cipher w-full py-3.5 text-sm disabled:opacity-60">
        ...
        <div class="hairline-t mt-6 pt-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <router-link to="/" class="text-xs font-bold text-teal-700 hover:text-teal-800 hover:underline shrink-0">
