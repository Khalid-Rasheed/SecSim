  ...
  <div class="max-w-md mx-auto px-4 py-14">
    ...
    <div v-if="auth.user" class="panel p-6">
      <div class="flex items-center gap-4 mb-5">
        <span
          class="w-14 h-14 rounded-full bg-cipher/10 border border-cipher/30 flex items-center justify-center"
        >
      ...
      <div class="hairline-t pt-4 flex items-center justify-between text-sm">
        <span class="text-muted"
          ><i class="fa-solid fa-calendar-days me-2"></i>{{ $t('profile.joined') }}</span
        >
        <span class="font-mono text-xs" dir="ltr">{{ (auth.user.created_at || '').slice(0, 10) }}</span>
      </div>
      <router-link
        to="/history"
        class="btn-ghost w-full py-2.5 text-sm mt-5 flex items-center justify-center gap-2"
      >
