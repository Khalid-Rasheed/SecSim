<template>
  ...
  <footer class="bg-white border-t border-[rgba(15,23,42,0.08)]">
    <div class="max-w-6xl mx-auto px-4 pt-12 pb-6">
      <div class="grid gap-10 md:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1.2fr]">
        ...
        <div>
          <div class="flex items-center gap-2.5 mb-4">
            <span class="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-pop"
              style="background: linear-gradient(135deg, #0d9488, #0f766e)">
              <i class="fa-solid fa-lock"></i>
            </span>
            <span class="font-display font-extrabold text-xl text-mist" dir="ltr">SecSim<span class="text-teal-600">_</span></span>
            <span class="inline-flex items-center gap-1 bg-teal-700/10 text-teal-800 border border-teal-700/20 text-[0.68rem] font-extrabold px-2.5 py-1 rounded-full" dir="ltr">
          ...
          <p class="inline-flex items-center gap-1.5 text-[0.72rem] font-bold text-amber-700 bg-amber-500/10 border border-amber-500/25 rounded-full px-3 py-1.5">
          ...
          <ul class="space-y-2.5 text-sm">
            ...
            <li>
              <router-link to="/history" class="text-muted hover:text-teal-700 font-semibold transition">
                <i class="fa-solid fa-clock-rotate-left text-[0.65rem] me-2 text-teal-600"></i>{{ $t('nav.history') }}
              </router-link>
            </li>
            <li>
              <router-link to="/login" class="text-muted hover:text-teal-700 font-semibold transition">
                <i class="fa-solid fa-right-to-bracket text-[0.65rem] me-2 text-teal-600"></i>{{ $t('nav.login') }}
              </router-link>
            </li>
          </ul>
        </div>
        ...
        <div>
          ...
          <ul class="space-y-2.5 text-sm">
            <li v-for="a in algos" :key="a">
              <router-link to="/simulator" class="inline-flex items-center gap-2 text-muted hover:text-teal-700 font-semibold transition">
                <span class="font-mono text-[0.7rem] bg-panel2 border border-[rgba(15,23,42,0.08)] rounded-md px-1.5 py-0.5" dir="ltr">{{ a }}</span>
              </router-link>
            </li>
          </ul>
        </div>
        ...
        <!-- OPEN SOURCE -->
        <div class="rounded-2xl bg-gradient-to-br from-teal-700/[0.07] to-amber-500/[0.08] border border-[rgba(15,23,42,0.08)] p-5">
          ...
          <div class="flex items-center gap-2 text-[0.72rem] font-bold text-teal-800">
            <span class="w-7 h-7 rounded-lg bg-teal-700/10 border border-teal-700/20 flex items-center justify-center">
      ...
      <div class="hairline-t mt-10 pt-5 flex flex-col sm:flex-row items-center gap-2 justify-between text-xs text-muted">
        <span class="font-mono" dir="ltr">SecSim © 2026 · {{ $t('footer.rights') }}</span>
        <span class="font-semibold"><i class="fa-solid fa-heart text-red-400 me-1.5"></i>{{ $t('footer.made') }}</span>
      </div>
    </div>
  </footer>
</template>
